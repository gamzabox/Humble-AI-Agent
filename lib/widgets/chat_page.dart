      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ...chat.models.map((m) => ListTile(
              ...
              trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                if (chat.activeModel?.id != m.id) TextButton(onPressed: () => chat.setActiveModel(m), child: const Text('Select')),
        ...
        child: Row(children: [
          SizedBox(
            ...
            child: ListView(children: [
              ListTile(selected: selected == 0, title: const Text('Models'), onTap: () => setState(() => selected = 0)),
              ListTile(selected: selected == 1, title: const Text('About'), onTap: () => setState(() => selected = 1)),
            ]),
          ),
          ...
          Expanded(
            child: Padding(
              ...
              child: selected == 0 ? const _ModelSettingsView() : const _AboutView(),
            ),
          )
        ]),
      ),
        ...
        if (chat.lastError != null)
          Container(
            ...
            child: Row(children: [
              ...
              TextButton(key: const Key('retry-button'), onPressed: chat.retryLast, child: const Text('Retry')),
            ]),
          ),
        Shortcuts(
          shortcuts: const {SingleActivator(LogicalKeyboardKey.enter, shift: true): SendMessageIntent()},
          child: Actions(
            actions: {SendMessageIntent: CallbackAction<SendMessageIntent>(onInvoke: (intent) { sendNow(); return null; })},
            child: Row(children: [
              Expanded(
                child: TextField(
              ...
              ElevatedButton(
                ...
                onPressed: () { if (isSending) { chat.cancel(); } else { sendNow(); } },
